...
class DonhangViews(APIView):
  ...
  def patch(self, request, idDonhang=None):
      ...
      if not order:
        ...
        data = Donhang.objects.get(id=idDonhang)
        if request.data.get('diachi') is not None:
          data.diachi = request.data.get('diachi') 
        if request.data.get('hoten') is not None:
          data.hoten = request.data.get('hoten') 
        if request.data.get('sodienthoai') is not None:
          data.sodienthoai = request.data.get('sodienthoai') 
        if request.data.get('isThanhtoan') is not None:
          data.isThanhtoan = Boolean(request.data.get('isThanhtoan'))
        if request.data.get('isHoanthanh') is not None:
          data.isHoanthanh = Boolean(request.data.get('isHoanthanh'))
        if request.data.get('thongtinthanhtoan_id') is not None:
          data.thongtinthanhtoan_id = int(request.data.get('thongtinthanhtoan_id'))
        data.save()
        return Response("thanhcong", status=status.HTTP_201_CREATED)
  ...
  def delete(self, request, idDonhang=None):
    ...
    if not order:
      ...
      return Response("Xoá đơn hàng thành công!", status=status.HTTP_201_CREATED)
